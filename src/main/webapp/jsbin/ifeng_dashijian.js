...
page.open('http://news.ifeng.com/a/20160307/47726772_0.shtml#p=1', settings, function(status) {
	...
	if (status === "success") {
		var result = page.evaluate(function() {
			var tgetUrl = 'http://news.ifeng.com/a/20160307/47726772_0.shtml#p=1';
			var uuid = GetUUID();
			var rec = [];
			var title = $('div.title>b').text();
			var that_listdata = window._listdata;
			var catId = 28; // 28看世界
			var imgs = [];
			var note = '';
			var cont = [];
			if(that_listdata && that_listdata.length>0){
				var img = that_listdata[0].timg;
				if(img && img != ''){
					imgs.push(img);
